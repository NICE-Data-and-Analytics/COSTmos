...
generate_unit_costs_hsc_tables <- function(report_year) {
  ...
  # Rename columns
  colnames(gp) <- c(
    "variable", "inc_direct_care_staff_cost_inc_qual_cost", "inc_direct_care_staff_cost_exc_qual_cost",
  ...
  gp <- gp |>
    ...
    dplyr::mutate(
      qualification_cost = dplyr::if_else(col %in% c("inc_direct_care_staff_cost_inc_qual_cost", "exc_direct_care_staff_cost_inc_qual_cost"), "including", "excluding"),
  ...
  colnames(training_costs_doctor) <- c(
    "job_title", "tuition", "living_expenses_or_lost_production_costs",
  ...
  gp_nurse <- tibble::tribble(
    ...
    dplyr::mutate(variable = dplyr::case_when(
      vars == "cost_hr" ~ "Cost per working hour",
