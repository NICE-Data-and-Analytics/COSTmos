...
uchsc_col_spec <- list(
  ...
  hospital_doctor = list(
    ...
    qualification_cost = reactable::colDef(show = FALSE),
    cost_per_working_hour = reactable::colDef(name = "Cost per working hour (\u00a3)",
                  cell = function(value) {
                    format(round(value, 2), big.mark = ",")
                  })
  ),
  ...
  training_costs_doctor = list(
    year = reactable::colDef(show = FALSE),
    job_title = reactable::colDef(name = "Job title"),
    tuition = reactable::colDef(name = "Tuition (\u00a3)",
                                   cell = function(value) {
                                     format(round(value, 2), big.mark = ",")
                                   }),
    living_expenses_or_lost_production_costs = reactable::colDef(name = "Living expenses/lost production costs (\u00a3)",
                     cell = function(value) {
                       format(round(value, 2), big.mark = ",")
                     }),
    clinical_placement = reactable::colDef(name = "Clinical placement (\u00a3)",
                     cell = function(value) {
                       format(round(value, 2), big.mark = ",")
                     }),
    placement_fee_plus_market_forces_factor = reactable::colDef(name = "Placement fee plus Market Forces Factor (\u00a3)",
                     cell = function(value) {
                       format(round(value, 2), big.mark = ",")
                     }),
    salary_inc_overheads_and_postgraduate_centre_costs = reactable::colDef(name = "Salary (inc overheads) and post-graduate centre costs (\u00a3)",
                     cell = function(value) {
                       format(round(value, 2), big.mark = ",")
                     }),
    total_investment = reactable::colDef(name = "Total investment (\u00a3)",
                              cell = function(value) {
                                format(round(value, 2), big.mark = ",")
                                }),
    expected_annual_cost_discounted_at_3pt5perc = reactable::colDef(name = "Expected annual cost discounted at 3.5% (\u00a3)",
                                                         cell = function(value) {
                                                           format(round(value, 2), big.mark = ",")
                                                           })
  ),
